...
I will illustrate the function `longest_increasing(perm, n)` with `perm = [1,10,5,2,7,8]` and `n = 6`. Also the function creates `longs = [1] * n`, a list with ``n`` ones because 1 is the minimal lenght of a increasing or decreasing sequence. Also we need to take a look of `padres = [-1,-1,-1,-1,-1,-1]`, the purpouse of padres is to store the index of all the 
